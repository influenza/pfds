defmodule MultiMapTestCase do
  ...
  test "should provide an empty check" do
    not_empty = [1,2,3] |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(&1, "hi", &2)))
    empty = %MultiMap{}

    assert MultiMap.empty? empty
    assert !MultiMap.empty? not_empty
  end

  test "should provide a check for existing keys" do
    keys = ["foo", "bar", "baz"]
    mmap = keys |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(&1, "hi", &2)))
  ...
  test "should merge two multimaps, preserving entries from both" do
    ...
    left_map = [3,2,1] |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(key, &1, &2)))
    right_map = [6,5,4] |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(key, &1, &2)))
  ...
  test "should provide means to delete keys" do
    keys = ["one", "two", "three"]
    mmap = keys |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(&1, 'val', &2)))
  ...
  test "provide an in-order reduction function" do
    mmap = [1,3,4,2] |> Enum.reduce(%MultiMap{}, &(MultiMap.insert(&1, &1, &2)))
  ...
  test "should provide access to entries" do
    mmap = [3,2,1] |> Enum.reduce(%MultiMap{}, &(MultiMap.insert("key", &1, &2)))
